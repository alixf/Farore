<?php
	...
	if($formSent)
	{
		...
		$websiteKeywords = $_POST['websiteKeywords'];
		...
		try
		{
			...
			$db = new PDO('mysql:host='.$databaseServer.';dbname='.$databaseBase, $databaseUsername, $databasePassword);
		...
		if($websiteName != strip_tags($websiteName) || empty($websiteName))
			$error['websiteName'] = true;
		if(!is_dir('modules/'.$defaultModule) || empty($defaultModule))
			$error['defaultModule'] = true;
		if($websiteDescription != strip_tags($websiteDescription))
			$error['websiteDescription'] = true;
		if($websiteKeywords != strip_tags($websiteKeywords))
			$error['websiteKeywords'] = true;
		if(!array_key_exists($websiteLanguage, $languages))
		...
		if(empty($error))
		{
			...
			{
				if(file_exists(dirname(__FILE__).'/modules/'.$module.'/install.php'))
					include_once dirname(__FILE__).'/modules/'.$module.'/install.php';
			}
			
			// If everything is still okay, finish installation
			if(empty($error))
			{
				$urlBasePath = $_SERVER['REQUEST_URI'][strlen($_SERVER['REQUEST_URI'])-1] == '/' ? $_SERVER['REQUEST_URI'] : dirname($_SERVER['REQUEST_URI']).'/';
				$websiteKeywordsArray = explode(',', $websiteKeywords);
				for($i = 0; $i < count($websiteKeywordsArray); $i++)
					$websiteKeywordsArray[$i] = '\''.trim($websiteKeywordsArray[$i]).'\'';
				
				// Write config file
				$configFileContent = file_get_contents(dirname(__FILE__).'/global/config.php');
				$configFileContent = str_replace('{{INTERNAL_BASE_PATH}}', realpath(dirname(__FILE__)).'/', $configFileContent);
				$configFileContent = str_replace('{{URL_BASE_PATH}}', $urlBasePath, $configFileContent);
				$configFileContent = str_replace('{{DEFAULT_MODULE}}', $defaultModule, $configFileContent);
	    		$configFileContent = str_replace('{{SQL_SERVER}}', $databaseServer, $configFileContent);
	    		$configFileContent = str_replace('{{SQL_BASE}}', $databaseBase, $configFileContent);
	    		$configFileContent = str_replace('{{SQL_USERNAME}}', $databaseUsername, $configFileContent);
				$configFileContent = str_replace('{{SQL_PASSWORD}}', $databasePassword, $configFileContent);
	    		$configFileContent = str_replace('{{ADMIN_USERNAME}}', $adminUsername, $configFileContent);
	    		$configFileContent = str_replace('{{ADMIN_PASSWORD}}', $adminPassword, $configFileContent);
			    $configFileContent = str_replace('{{PAGE_TITLE}}', $websiteName, $configFileContent);
				$configFileContent = str_replace('{{PAGE_DESCRIPTION}}', $websiteDescription, $configFileContent);
				$configFileContent = str_replace('\'{{PAGE_KEYWORDS}}\'', 'array('.implode(', ', $websiteKeywordsArray).')', $configFileContent);
				$configFileContent = str_replace('{{PAGE_LANGUAGE}}', $websiteLanguage, $configFileContent);
				$configFileContent = str_replace('{{PAGE_READINGDIR}}', 'ltr', $configFileContent);
				
				$configFile = fopen(dirname(__FILE__).'/global/config.php', 'w');
				fwrite($configFile, $configFileContent);
				fclose($configFile);
				
				// The installation is finished, replace this file with the home
				unlink(__FILE__);
				rename(dirname(__FILE__).'/home.php', __FILE__);
		...
		<div class="content">
			<form method="POST" action=".">
				...
				<div class="item<?php if(isset($error['websiteName'])) echo ' error'; ?>">
					<label>Nom du site</label><input type="text" name="websiteName" value="<?php echo $websiteName; ?>" />
				...
				<div class="item<?php if(isset($error['websiteDescription'])) echo ' error'; ?>">
					<label>Description du site</label><textarea name="websiteDescription"><?php echo $websiteDescription; ?></textarea>
				...
				<div class="item<?php if(isset($error['websiteKeywords'])) echo ' error'; ?>">
					<label>Mots clés du site</label><input type="text" name="websiteKeywords"  value="<?php echo $websiteKeywords; ?>"/>
				...
				<div class="item<?php if(isset($error['websiteLanguage'])) echo ' error'; ?>">
					<label>Langage du site</label><select name="websiteLanguage">
						...
						foreach($languages as $language => $languageName)
							echo '<option value="'.$language.'"'.($language == $websiteLanguage ? ' selected="selected"' : '').'>'.$languageName.'</option>';
						?>
					</select>
